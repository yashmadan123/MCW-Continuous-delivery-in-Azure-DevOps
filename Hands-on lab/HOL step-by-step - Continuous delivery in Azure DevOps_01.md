...
2. You should arrive at the `Security & analysis` blade under the `Settings` tab. Enable `Dependabot security updates`.
    ...
    > **Note**: The alerts for the repository may take some time to appear. The rest of the steps for this task rely on the alerts to be present. You can continue with the next exercise as this is a independent task and doesn't effect the lab. Please visit this task later and complete the task.
