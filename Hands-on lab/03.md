...
10. Repeat step 9 and add another `Docker Compose` task and configure it with the following fields:
    ...
    The final file should be similiar like the beloow:
    
     ![Docker Compose Task definition in the AzureDevOps pipeline.](media/final1.png "Docker Compose Task")

11. Now click on Save and run from th eright core=ner to run the build. New docker images will be built and pushed to the GitHub package registry.
...
15. Navogate to Pipelines and select the pipeline you create in last step and click `Edit` mode, and then select the `Variables` button on the top-right corner of the pipeline editor. 
...
Congrtulation, You have completed this workshop.
-------
