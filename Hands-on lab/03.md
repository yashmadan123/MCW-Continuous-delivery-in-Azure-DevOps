...
3. In your GitHub lab files repository, navigate to the `Secrets` > `Actions` blade in the `Settings` tag and create a new repository secret named `AZURE_CREDENTIALS`. Paste the JSON output copied in the previous step to the secret value and save it.
...
4. On the New Service Conenction tab, search for Docker and select **Docker Registry**, then click on **Next**.
