...
6.  Remove all the content from the YAML and replace it with below content. Make sure to change the student prefix with **<inject key="Deploymentid" />** for the last action in the `build` job.
...
13. Create a new `Azure Resource Manager` service connection and choose `Service Principal (manual)`.
...
15. Open the build pipeline in `Edit` mode, and then select the `Variables` button on the top-right corner of the pipeline editor. 
...
17. Modify the build pipeline YAML to split into a build stage and a deploy stage, as follows. Make sure to replace **arguments** value with **<inject key="Deploymentid" />**.
...
19. From the vertical ellipsis menu button in the top-right corner, select `Approvals and checks`.
...
20. Add an `Approvals` check.  Add your account as an `Approver` and create the check.

    ![Adding an account as an `Approver` for an Approvals check.](media/hol-ex3-task3-step19-1.png "Checks selection")

21. Run the build pipeline and note how the pipeline waits before moving to the `DeployProd` stage. You will need to approve the request before the `DeployProd` stage runs.
