...
6. Change the `name` property to `Docker Compose Build and Deploy`. Remove all the content from the YAML and replace with below content. 

   Make sure to change the student prefix with **<inject key="Deploymentid" />** for the last action in the `build` job.
...
17. Modify the build pipeline YAML to split into a build stage and a deploy stage, as follows.
