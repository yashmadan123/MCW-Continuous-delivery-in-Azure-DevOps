...
6. Open the `seed-cosmosdb.ps1` PowerShell script in the `C:\Workspaces\lab\mcw-continuous-delivery-lab-files\infrastructure` folder of your lab files GitHub repository and replace `$studentprefix` variable value with **<inject key="Deploymentid" />**, add `$githubRepo = "Your gihub repository name here"` variable, and replace the last line with `docker.pkg.github.com/$githubAccount/$githubRepo/fabrikam-init:main`. 
   ...
   Update the `$githubAccount`, `$githubRepo` variables with your GitHub account name, GitHub lab files repository name and include `:main` at the end of the file. 
    ...
    ![Azure Resource Group containing cloud resources to which GitHub will deploy containers via the workflows defined in previous steps.](media/update4.png "Azure Resource Group")
